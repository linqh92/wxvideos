...
同一轮选定多个独立选题时，按“一条内容一份交接”处理。例如同时选定第 2、4 题，当前选题对话分别生成两份 `选题交接`，用户再为两份文件各建一个文案对话。两个选题可以共享原推荐批次，但必须分别拥有 `topic_id`、`content_id`、selected 反馈事件和最终交付文件；只有用户明确要求合并成一篇并确认合并后的唯一选题时，才使用一份 Handoff。

选题被正式选定后建立一个稳定 `content_id`，后续文案、可选视觉、发布和归档始终复用。规则见 `shared/schemas/content-identity-schema.md`。
...
候选池是个人多账号维护中的备查材料，状态使用“待核验、可推荐、已发布、已放弃”。用户选题由 `selected` 反馈记录，内容身份由 `content_id` 记录；已推荐题不会因仍为“可推荐”而自动再次入选。上轮未选题默认退出下一轮，明确否定按用户表达的题目或方向范围执行。旧题回归需要用户主动指定或新的材料、条件、判断价值。
...
Copy approval produces a canonical pending Repo document with an empty publication date. Complete manual structural checks before web delivery; no script or Codex approval is required. `check-delivery` is an optional utility. Topic planning, copywriting, file delivery and visual planning remain in the web workflow; Codex validates internally when a Repo operation is requested. Archive only after actual publication is confirmed; `check` remains an archive-plan check. An explicit actual date can be supplied with `--confirmed-publish-date` to check, sync or verify a structurally valid pending document. Never use confirmation time as the publication date.

For generation in another project or service, ensure its accessible rule source contains the current Delivery Gate and `check-delivery` contract. Local edits do not update external project copies automatically. Update the accessible rule source when rules change; complete rules already read in a conversation may be reused unless an update or conflict requires refreshing them. If GitHub access is unavailable, use complete project-source copies.
