...
When the user explicitly references an incoming Handoff, validate it with `shared/schemas/handoff-packet-schema.md` and treat its confirmed topic as the sole prior-stage working result. Preserve its topic-origin fields, selected feedback and unresolved `pending_repo_actions` as Repo-only metadata; do not use that metadata as creative context. Do not load `topic-planning`, recommendation history, the three planning indexes, rejected topics, prior drafts or the earlier chat. Read only source paths explicitly required by the Handoff when they are necessary for the current copy.
...
After delivering the requested titles, body copy, or complete package, stop. Do not re-plan the topic, write to history, update the content map, execute repository actions, or produce visual assets.

When the user explicitly confirms the final copy, make sure the Repo document has one unambiguous final title; if multiple title options remain, ask the user to select one or explicitly authorize a choice before finalizing. Create or reuse its stable `content_id` under `shared/schemas/content-identity-schema.md`, then generate one `Repo内容文档` with the publication-backfill fields required by `shared/schemas/confirmed-copy-delivery-schema.md` and stop.

At finalization only, read the current account's `内容库/00-首页与维护规则/固定选项库.md` and use the confirmed copy plus already loaded account context to complete the `Repo 操作载荷`. Populate its archive classification, summary, audience and pain-scene fields once; keep `extension_topics` and `related_content` as empty arrays unless the user has explicitly supplied save-ready values. The payload must conform to `shared/schemas/repo-operation-schema.md` so the later Codex sync can execute without reinterpreting the copy.

If the copy came from a topic Handoff, carry its topic trace, selected feedback, sync status and every unresolved `pending_repo_actions` entry into the Repo document without changing IDs or reducing payloads. If the user supplied the topic or reference directly without a project recommendation, mark topic feedback `not_applicable` and do not invent a batch or feedback event. Do not ask for or generate a Handoff. Tell the user to reference the Repo document in a writable Codex task and request “同步” or “录入” after completing publication; that instruction executes the document's archive action.
