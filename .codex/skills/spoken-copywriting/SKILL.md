...
结合账号简报，在内部形成一段简短的创作意图：这条内容要吸引哪类客户，在什么处境下；有效 Handoff 已确认的 Acquisition Anchor 是什么，或者在没有 Handoff 时，当前材料中哪个有依据的场景、结果、数字、利益、决策张力、处理障碍或确实存在的误解最值得他继续听；这位出镜者凭什么值得信任。
...
有效 Handoff 已提供 Acquisition Anchor 时，保留该价值中心，只决定怎样通过口播把它讲出来。可以调整用词、标题、信息顺序、开场形式、数字出现时机、解释深度、人物表达和节奏。只有原获客核心与核验事实冲突、超出信息身份允许的表达强度、无法在口播容量内可靠承载，或用户明确要求更换切口时，才改变获客核心。
...
| Handoff fidelity | 如果任务通过已确认 Handoff 进入，最终标题、开场和正文是否保留或强化了 Acquisition Anchor？如发生实质改变，是否由事实可靠性、信息身份边界、载体限制或用户明确要求所驱动？ |
...
- 只要标题：仅交付标题选项。
- 只要口播：仅交付最终口播稿。
- 完整口播包：三个表达方式不同的获客标题、一个常规搜索标题、一个短标题、一篇最终口播稿；有 Handoff 时，全部标题保留已确认的 Acquisition Anchor。
