...
本文件定义内容从“选题已选定”开始跨阶段复用的稳定身份。`content_id` 是关联键，不是内容状态，也不代替候选、推荐、发布或归档状态。

## 创建时点

- 用户明确选定某个选题时创建；仅推荐、浏览、跳过或待确认时不创建。
- 同一推荐批次中选定多个彼此独立的选题时，每个选题分别创建不同的 `content_id`；共同的 `recommendation_batch_id` 只表示来源相同，不得作为共享内容身份的理由。
...
一条独立内容固定对应一个 `topic_id`（项目推荐题存在时）、一个 `content_id`、一份当前阶段 Handoff 和一个下游文案对话。多个方向只有在用户明确要求合并且已确认合并后的唯一选题时，才作为一条内容建立一个 `content_id`。
